	...
	def execute (self):
		...
		if self.decoded_inst['type'] == 0:
			...
			#Divisão
				...
				self.dprint("div r"+str(self.decoded_inst['r_dest'])+", r"+str(self.decoded_inst['r_op1'])+", r"+str(self.decoded_inst['r_op2']))
				self.regs[ self.decoded_inst['r_dest'] ] = self.regs[ self.decoded_inst['r_op1'] ] / self.regs[ self.decoded_inst['r_op2'] ]
			
			#cmp_equal  
			elif self.decoded_inst['opcode'] ==4:
				self.dprint('cmp_equal r{}, r{},r{}'.format(self.decoded_inst['r_dest'],self.decoded_inst['r_op1'],self.decoded_inst['r_op2']))
				self.regs[ self.decoded_inst['r_dest']] = 1 if self.regs['r_op1'] == self.regs['r_op2'] else 0

			#cmp_nequal  
			elif self.decoded_inst['opcode'] ==5:
				self.dprint('cmp_nequal r{}, r{},r{}'.format(self.decoded_inst['r_dest'],self.decoded_inst['r_op1'],self.decoded_inst['r_op2']))
				self.regs[ self.decoded_inst['r_dest']] = 1 if self.regs['r_op1'] != self.regs['r_op2'] else 0
			
			#load
			elif self.decoded_inst['opcode'] == 15:
				self.dprint('load r{},  r{}, r{}'.format(self.decoded_inst['r_dest'],self.decoded_inst['r_op1'],self.decoded_inst['r_op2']))
				self.regs['r_dest'] = self.memory_load[self.regs['r_op1']]
			#store
			elif self.decoded_inst['opcode'] == 16:
				self.dprint('Store r{},  r{}, r{}'.format(self.decoded_inst['r_dest'],self.decoded_inst['r_op1'],self.decoded_inst['r_op2']))
				self.memory_store[self.regs['r_op1'],self.regs['r_op2']]


	
			# TODO:
			# Adicionar aqui as instrucoes do tipo R.
			# Para acessar a memoria, olhar as funcoes:
			# self.memory_load
			# self.memory_store
			...
			# Adicionar aqui as instrucoes do tipo I.
				...
				self.dprint("Jump cond "+str(self.decoded_inst['imed']))
				self.reg_pc = self.decoded_inst['i_imed'] if self.reg_pc == 1 else self.reg_pc
				self.dprint('jump_cond, self.reg_pc = {}'.format(self.reg_pc))

			#mov
			elif self.decoded_inst['opcode'] == 3:
				self.dprint('mov r{}, {}'.format(self.decoded_inst['i_reg'],self.decoded_inst['i_imed']))
				self.regs =  self.decoded_inst['i_imed']
